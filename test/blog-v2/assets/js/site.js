...
function resolveArticleSlugFromHref(href) {
  ...
  return window.__SITE_DATA__.linkMap && window.__SITE_DATA__.linkMap[sanitized];
}
...
function resolveRuntimeAsset(rawHref, article) {
  ...
  return window.__SITE_DATA__.site.root + '/assets/images/articles/' + article.slug + '/' + fileName;
}
...
async function discoverArticleEntries() {
  const rootPath = window.__SITE_DATA__.site.root + '/articles/';
  const visitedDirs = new Set();
  const discovered = new Map();

  async function visit(dirUrl) {
    const absoluteDirUrl = new URL(dirUrl, window.location.origin).toString();
      ...
      if (/.md$/i.test(resolvedUrl.pathname)) {
        ...
        if (!discovered.has(slug)) {
          discovered.set(slug, {
            slug,
            mdPath,
            mdUrl: window.__SITE_DATA__.site.root + '/articles/' + mdPath
          });
        }
        return;
      }
...
async function loadRuntimeArticles() {
  const manifestMap = new Map((window.__SITE_DATA__.articles || []).map((article) => [article.slug, {
    ...
    mdUrl: window.__SITE_DATA__.site.root + '/articles/' + article.mdPath
  }]));
